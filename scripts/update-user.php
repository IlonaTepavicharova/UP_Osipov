...
$update_users_sql = sprintf(
    "UPDATE `users` SET `first_name` = '%s', `last_name` = '%s', `username` = '%s', `password` = '%s', `bio` = '%s', `email` = '%s',`url_site` = '%s', `vk` = '%s'  WHERE `user_id` = %d",
...
// выполнение запроса на обновление данных таблицы users
...
// проверка отсутствия ошибки при отправке изображения 
        if ($_FILES["user_pic"]["error"] != 0) {
            switch ($_FILES["user_pic"]["error"]) {
                case 1:
                    $system_error_message = "Превышен максимальный размер файла указанный в файле php.ini";
                    break;
                case 2:
                    $system_error_message = "Превышен максимальный размер файла указанный в форме HTML";
                    break;
                case 3:
                    $system_error_message = "Была отправлена только часть файла";
                    break;
                case 4:
                    $system_error_message = "Файл для отправки не был выбран";
            }
    ...
    if (is_uploaded_file($_FILES["user_pic"]["tmp_name"]) == 0) {
        $system_error_message = "Запрос на отправку локального файла: " . $_FILES["user_pic"]["tmp_name"];
    ...
    if (!getimagesize($_FILES["user_pic"]["tmp_name"])) {
        ...
        header("Location: show-error.php?error_message=Вы выбрали файл, для своего фото, который не является изображением&system_error_message=$system_error_message");
        exit;
    }
...
= '%s'WHERE `user_id` = %d",
    ...
    if (!$res) {
        header("Location: show-error.php?error_message=Ошибка обновления изображения пользователя&system_error_message="
            . $mysqli->error);
        exit;
    }
    ...
    header("Location: show-error.php?error_message=Ошибка обновления данных пользователя&system_error_message=" . $mysqli->error);
}
exit;
?>